...
- discussion: Diskussion (Code, Thema, Flags, aktueller Schritt, aktueller Diskussionspunkt).
...
- frame_of_values: Werterahmen je Diskussion (value_id = 0-basierte Position in `values.ts`).
- user_values: Werteauswahl je User und Diskussion (value_id = 0-basierte Position in `values.ts`).
...
CREATE TABLE discussion (
  ...
  FOREIGN KEY (current_discussion_point_id) REFERENCES discussionpoints(dp_id) ON DELETE SET NULL
);
